...
public class BinaryTreeOperations {
    ...
    private void collectParentNodesRec(BinaryTreeNode node, List<Integer> out) {
        if (node == null) return;
        if (!node.isLeaf()) out.add(node.getValue());
    ...
    /**
     * Recolecta pares de hermanos (cada par representado como una lista de dos enteros).
     */
    ...
    private int getLevelRec(BinaryTreeNode node, int value, int level) {
        ...
        if (node.getValue() == value) return level;
    ...
    /**
     * Devuelve una lista de nodos por nivel (lista de listas; cada sublista contiene
    ...
    /**
     * Longitud de camino interno (LCI): suma de las profundidades de todos los nodos.
     */
    ...
    /**
     * Media de longitud de camino interno (LCIM): LCI / n (n = número de nodos, si n>0).
     */
