...
public class HuffmanAlgorithmOperations {
    ...
    /**
     * Calcula la longitud de camino ponderada (costo Huffman): suma de
     * frecuencia * profundidad para cada símbolo (LCI ponderada).
     *
     * @param root raíz del árbol de Huffman
     * @return longitud de camino ponderada
     */
    public static long calculateWeightedPathLength(HuffmanNode root) {
        if (root == null) return 0L;
        // caso especial: un solo símbolo → consideramos profundidad 1 (código "0")
        if (root.getLeft() == null && root.getRight() == null) {
            return (long) root.getFrequency();
        }
    ...
    private static long calculateWeightedPathLengthRec(HuffmanNode node, int depth) {
        ...
        if (node.getLeft() == null && node.getRight() == null) {
            return (long) node.getFrequency() * depth;
        }
