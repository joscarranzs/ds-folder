...
public class HuffmanAlgorithmVisualizer extends BorderPane {
    ...
    public void showResult(
        ...
        for (Map.Entry<Character, Integer> entry : sortedFrequencies.entrySet()) {
            addRow(formatChar(entry.getKey()), String.valueOf(entry.getValue()));
        }

        addTitle("Codigos");
        Map<Character, String> sortedCodes = new TreeMap<>(codes);
        for (Map.Entry<Character, String> entry : sortedCodes.entrySet()) {
            addRow(formatChar(entry.getKey()), entry.getValue());
        }
    }
    ...
    private NodeElement createNodeElement(HuffmanNode node, double x, double y) {
        ...
        String text = isLeaf
                ? formatChar(node.getCharacter()) + ":" + node.getFrequency()
    ...
    private String formatChar(char character) {
        if (character == ' ') {
            return "espacio";
        }
